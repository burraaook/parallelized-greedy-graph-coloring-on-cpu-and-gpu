...
int parallel_graph_coloring(std::string filename, int num_threads, std::ofstream &file);
...
{
    if (argc < 2) {
        std::cerr << "Usage: ./main <algorithm_name> <dataset_path> [num_threads]" << std::endl;
        return 1;
    }
    ...
    if (algorithm == "run_tests") {
        ...
        if (argc != 3) {
            std::cerr << "Usage: ./main basic_greedy <dataset_path>" << std::endl;
            return 1;
        }
        ...
        if (argc != 3) {
            std::cerr << "Usage: ./main bitwise_greedy <dataset_path>" << std::endl;
            return 1;
        }
        ...
        if (argc != 4) {
            std::cerr << "Usage: ./main p_bitcolor <dataset_path> <num_threads>" << std::endl;
            return 1;
        }
        ...
        parallel_graph_coloring(dataset, num_threads, bitcolor_parallel_results_file);
...
int basic_greedy(std::string filename, std::ofstream &file)
{
    ...
    std::cout << "Number of colors used: " << g.getNumColor() << std::endl;
...
int bit_wise_greedy_normal(std::string filename, std::ofstream &file)
{
    std::cout << "\nbit_wise_greedy_normal__ " << std::endl;
    std::cout << "dataset: " << filename << std::endl;
...
int parallel_graph_coloring(std::string filename, int num_threads, std::ofstream &file)
{
    std::cout << "\nparallel_graph_coloring__ " << std::endl;
    std::cout << "dataset: " << filename << std::endl;

    ParallelGraph g(filename, num_threads);
